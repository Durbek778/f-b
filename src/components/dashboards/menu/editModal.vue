...
const handleModal = async (action: 'CLOSE' | 'SAVE') => {
    if (action == 'CLOSE') {
        ...
        if (valid) {
        const  {menuId,menuName,menuCategory, menuDiscountPrice, menuMemo,menuPrice,menuTotalPrice, menuImgUrl } = menu.value
            if(imageEdit.value){

                menuStore
                .updateMenu({menuName,menuCategory, menuDiscountPrice, menuMemo,menuPrice,menuTotalPrice })
                .then(() => {
                ...
                menuStore
                .updateMenu({ menuId,menuName,menuCategory,menuDiscountPrice:+menuDiscountPrice, menuMemo,menuPrice:+menuPrice,menuTotalPrice:+menuTotalPrice,menuImgUrl })
                .then(() => {
                    alertStore.addAlert('SUCCESS', `메뉴가 성공적으로 업데이트되었습니다`);
                    emit('save', 'EDIT');
                });
    ...
    () => {
        ...
        if (props.isOpen) {
            menu.value = {
                ...
                menuCategory: props?.menu?.menuCategory,
                menuDiscountPrice: props?.menu?.menuDiscountPrice,
                menuImgUrl:props?.menu?.menuImgUrl,
                menuPrice: props?.menu?.menuPrice,
                menuMemo:props?.menu?.menuMemo,
                menuTotalPrice:props?.menu?.menuTotalPrice,
