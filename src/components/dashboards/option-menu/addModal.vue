...
const handleModal = async (action: 'CLOSE' | 'SAVE') => {
    if (action == 'CLOSE') {
        ...
        if (valid) {
        const  {optionMenuName,optionMenuCategory, optionMenuDiscountPrice, optionMenuMemo,optionMenuPrice,optionMenuTotalPrice, optionMenuImgUrl } = optionMenu.value
            if(imageEdit.value){
                optionMenuStore
                .postOptionMenu({menuId:props.menuId  ,optionMenuName,optionMenuCategory, optionMenuDiscountPrice, optionMenuMemo,optionMenuPrice,optionMenuTotalPrice, optionMenuImgUrl } )
                .then(() => {
...
const optionMenu = ref<IOptionMenu>({
    menuId: 0,
    optionMenuId: 0,
    optionMenuCategory: "",
    optionMenuName: "",
                ...
                <v-form ref="form" lazy-validation>
                    <v-row class="pt-2">
                        <v-col cols="12" sm="12">
                            <v-text-field
                                label="메뉴 이름"
                                v-model="optionMenu.optionMenuName"
                                variant="outlined"
                                class="users__input"
                                hide-details
                        ...
                        <v-col cols="12" sm="12">
                            <v-select
                                label="메뉴 카테고리"
                                v-model="optionMenu.optionMenuCategory"
                                variant="outlined"
                                class="users__input"
                                :items="[
                                    { key: '빵', value: '빵'},
                        ...
                        <v-col cols="6" sm="6">
                            <v-text-field
                                label="가격"
                                v-model="optionMenu.optionMenuPrice"
                                variant="outlined"
                                class="users__input"
                                hide-details
                            ></v-text-field>
                        ...
                        <v-col cols="6" sm="6">
                            <v-text-field
                                label="할인 가격"
                                v-model="optionMenu.optionMenuDiscountPrice"
                                variant="outlined"
                                class="users__input"
                                hide-details
                            ></v-text-field>
                        ...
                        <v-col cols="12" sm="12">
                            <v-text-field
                                label="메뉴 총가격"
                                v-model="optionMenu.optionMenuTotalPrice"
                                variant="outlined"
                                class="users__input"
                        ...
                        <v-col cols="12" sm="12">
                            <v-textarea
                                label="메뉴 메모"
                                v-model="optionMenu.optionMenuMemo"
                                variant="outlined"
                                class="users__input"
                                :rules="discountValueRules"
                                hide-details
