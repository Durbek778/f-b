...
import { useHistoryStore, type IHistoriesParam } from '@/stores/history';
import { onMounted } from 'vue';
import { useRouter } from 'vue-router';
const page = ref({ title: '히스토리' });
...
const search = () => {
    historyStore.getHistories({ ...params.value, offset: offset.value });
};
const setPage = (page: number) => {
    if (currentPage.value == page) return;
    currentPage.value = page;
    router.replace({ path: '/history', query: { page: Number(page) } });
    search();
};
